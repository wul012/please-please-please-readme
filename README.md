...
本项目集中的每个项目，几乎都包含超级详细的README与代码讲解，并且会在 `a/`或者'b' 文件夹下保留每次运行时的截图记录。多数项目版本已到100。本仓库也在准备一些具体的小讲解md，推荐阅读。

不过需要提前说明：这些讲解内容并不是从最基础的概念开始写起的。它们更侧重于项目过程、代码逻辑、运行结果和关键知识点的说明。如果缺少相关基础知识与对名词含义的理解，阅读时可能会觉得某些地方跳得比较快。
